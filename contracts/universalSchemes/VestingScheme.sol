...
contract VestingScheme is UniversalScheme, ExecutableInterface {
    ...
    mapping(address=>mapping(bytes32=>Agreement)) public organizationProposals;
    ...
    */
    ...
    returns(bytes32)
    {
        ...
        for (uint cnt = 0; cnt<_signersArray.length; cnt++) {
            organizationProposals[_avatar][proposalId].signers[_signersArray[cnt]] = true;
        }
        // Write parameters:
        organizationProposals[_avatar][proposalId].token = Avatar(_avatar).nativeToken();
        organizationProposals[_avatar][proposalId].beneficiary = _beneficiary;
        organizationProposals[_avatar][proposalId].returnOnCancelAddress = _returnOnCancelAddress;
        organizationProposals[_avatar][proposalId].startingBlock = _startingBlock;
        organizationProposals[_avatar][proposalId].amountPerPeriod = _amountPerPeriod;
        organizationProposals[_avatar][proposalId].periodLength = _periodLength;
        organizationProposals[_avatar][proposalId].numOfAgreedPeriods = _numOfAgreedPeriods;
        organizationProposals[_avatar][proposalId].cliffInPeriods = _cliffInPeriods;
        organizationProposals[_avatar][proposalId].signaturesReqToCancel = _signaturesReqToCancel;
    ...
    */
    function execute(bytes32 _proposalId, address _avatar, int _param) public returns(bool) {
        ...
        Agreement memory proposedAgreement = organizationProposals[_avatar][_proposalId];
